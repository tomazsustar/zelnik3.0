 ...
 */
class VsebineModelEvent extends JModelItem {
		...
		if ($prispevek) {
			...
			try {
				...
				$query = $db->getQuery(true);
					...
					$query->join('INNER', '`nize01_cinovicomat`.vs_events as a ON c.ref_id = a.id');
				...
				if($version){
					...
					$query->select('s.*');
					$query->join('INNER', "`nize01_cinovicomat`.vs_content AS c ON c.ref_id = s.id AND c.type =  'image'" );
					$query->join('INNER', "`nize01_cinovicomat`.vs_content_content AS cc ON c.id = cc.ref_content_id AND (cc.position='right' OR cc.position='head') AND cc.content_id=$data->id");
					$query->order("cc.ordering ASC");
					$db->setQuery($query);
