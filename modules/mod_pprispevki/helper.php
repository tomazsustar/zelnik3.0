...
class Prispevek {
	...
	private function GetAllTags() {
		...
		if($version){
			$query = "SELECT c.id, c.tag FROM nize01_cinovicomat.vs_content AS c
...
class PovezaniPrispevki {
	...
	function __construct($Prispevek, $Omejitev) {
		...
		if($version){
			$query = 
			"select common_tags, all_tags - common_tags as not_common_tags, c.id, c.name from nize01_cinovicomat.vs_content as c
